...
$dropdown_currencies = explode(',', $params->get('dropdown_currencies'));
...
$lastUpdateDate      = gmdate('M d, Y H:i', strtotime($currenciesModel->getLastUpdateDate($this->base->code)));
...
$datelang = str_replace("-", "_", $lang->getTag()) . '.UTF-8';
...
$pairtitle = sprintf(JText::_('CURRENCIES_6'), JText::_('MAIN_' . strtoupper($this->base->name)), $this->base->code,
    JText::_('MAINMC_' . strtoupper($this->base->name)));
    ...
    <div class="col-xs-12 col-md-9 col-lg-9">
        <div class="row">
            ...
            <div class="col-sm-12" style="margin-bottom:10px">
                ...
                <h1> <?php printf(JText::_('COM_CURRENCIES_LANDING_PAGE_TITLE'),
                        (JText::_('MAIN_' . strtoupper($this->base->name)) . " " . $this->base->code . "")) ?></h1>
                <?php echo JText::_('COM_CURRENCIES_LAST_UPDATE'); ?><?php setlocale(LC_ALL,
                    sprintf($datelang, $glang, strtoupper($glang)));
                echo strftime('%e %B %Y %H:%m', strtotime($lastUpdateDate)) . " UTC." ?>
            </div>
        </div>
    ...
    <?php if ($detect->isTablet() && $i > 7) {
                 ...
                 " title="<?php echo $currency->name ?>">
                    <a href="<?php echo JRoute::_('index.php?option=com_currencies&view=pairs&base=' . $this->params->get('primary_currency') . '&to=' . $currency->code . '&value=' . $this->defaultValue . '&Itemid=' . $this->params->get('pairs_itemid')); ?>">
                        ...
                        <img src="components/com_currencies/assets/images/blank.gif"
                             class="flag flag-<?php echo strtolower(substr($currency->flag, 0, 2)); ?>"
                             alt="<?php echo $currency->code . " " . $currency->name ?>"/> <?php echo $currency->code ?>
        ...
        <div class="row rate-row">
            <div class="col-sm-2 base-flag">
                ...
                <img src="components/com_currencies/assets/images/blank.gif" style="margin-top:-12px"
                     class="flag flag-<?php echo strtolower(substr($this->base->flag, 0, 2)); ?>"
                     alt="<?php echo $currency->code . " " . $currency->name ?>"/>
            ...
            <?php for ($i = 0; $i < count($this->currencies); $i++) { ?>
                ...
                if (count($this->baseRate) > 0) {
                    if (floatval($this->baseRate[1]) >= 1000) {
                        ...
                        if (floatval($this->baseRate[1]) >= 100) {
                            $newRate = $rate * 1000;
                        } else {
                            if (floatval($this->baseRate[1]) >= 10) {
                                $newRate = $rate * 100;
                            } else {
                                $newRate = $rate;
                            }
                ...
                <?php if ($detect->isTablet() && $i > 7) {
                 ...
                 ">
                    ...
                    if (trim($options[0]->currency_name) == 'Dollar') {
                        ...
                        <p> <?php
                            if ($this->base->code == "BTC") {
                                echo decima0($newRate);
                            } else {
                                echo $options[0]->currrency_symbol . decima2($newRate);
                            }
                        ...
                        <p><?php
                            ...
                            if ($this->base->code == "BTC") {
                                ...
                                echo decima2($newRate) . $options[0]->currrency_symbol;
                            }
            ...
            <div class="col-sm-12">
                ...
                <form class="form-inline form-validate"
                    ...
                    <div class="form-group">
                        ...
                        <input type="number" step="0.000001" value="1" name="value" class="form-control required"
                               id="inputamount" placeholder="<?php echo JText::_('AMOUNT') ?>" lang="en-150">
                    ...
                    <div class="form-group">
                        ...
                        <select name="to" class="required input-large" id="currencyselect_tod">
                            <optgroup label="<?php echo JText::_('COM_CURRENCIES_RECENT') ?>">
                                <?php foreach ($top_currencies as $top_currency): ?>
                                    <option <?php echo $top_currency->code == $this->defaultCurrency->code ? 'selected="selected"' : '' ?>
                                            value="<?php echo $top_currency->code ?>"
                                            data-image="<?php echo Juri::root() . 'images/flags/' . $top_currency->flag ?>">
                                        <?php echo $top_currency->code ?>
                                        - <?php echo JText::_('MAIN_' . strtoupper($top_currency->name)) ?>
                            ...
                            <optgroup label="<?php echo JText::_('COM_CURRENCIES_ALLCURR') ?>">
                                <?php foreach ($this->allCurrencies as $cto) { ?>
                                    <option <?php echo $cto->code == $this->defaultCurrency->code ? 'selected="selected"' : '' ?>
                                            value="<?php echo $cto->code ?>"
                                            data-imagesrc="<?php echo Juri::root() . 'images/flags/' . $cto->flag ?>"
                                            data-description="<?php echo $cto->name ?>"><?php echo $cto->code ?>
        ...
        <div class="col-sm-12 well center shandow">
            ...
            if ($this->defaultCurrency->code == "TRY") {
                $lira = "TL";
            } else {
                $lira = "";
            }
            echo sprintf(JText::_('CURRENCIES_7A'), $funt, JText::_('MAIN_' . strtoupper($this->defaultCurrency->name)),
                $lira, JText::_('MAINMC_' . strtoupper($this->defaultCurrency->name)),
                JText::_('MAINMC_' . strtoupper($this->base->name)));
            ...
            if (($this->current[1] * $this->value) < 0.1) {
                $decimalx = decima7($this->current[1] * $this->value);
            } elseif (($this->current[1] * $this->value) < 1) {
                $decimalx = decima5($this->current[1] * $this->value);
            } elseif (($this->current[1] * $this->value) < 10) {
                $decimalx = decima4($this->current[1] * $this->value);
            } elseif (($this->current[1] * $this->value) < 100) {
                $decimalx = decima3($this->current[1] * $this->value);
            } elseif (($this->current[1] * $this->value) < 1000) {
                $decimalx = decima2($this->current[1] * $this->value);
            } elseif (($this->current[1] * $this->value) < 10000) {
                $decimalx = decima1($this->current[1] * $this->value);
            } else {
                $decimalx = decima0($this->current[1] * $this->value);
            }

            if ($this->current[1] < 0.1) {
                $decimalx1 = decima7($this->current[1]);
            } elseif ($this->current[1] < 1) {
                $decimalx1 = decima5($this->current[1]);
            } elseif ($this->current[1] < 10) {
                $decimalx1 = decima4($this->current[1]);
            } elseif ($this->current[1] < 100) {
                $decimalx1 = decima3($this->current[1]);
            } elseif ($this->current[1] < 1000) {
                $decimalx1 = decima2($this->current[1]);
            } elseif ($this->current[1] < 10000) {
                $decimalx1 = decima1($this->current[1]);
            } else {
                $decimalx1 = decima0($this->current[1]);
            }

            if (($this->inverse[1]) < 0.1) {
                $decimalx2 = decima7($this->inverse[1]);
            } elseif (($this->inverse[1]) < 1) {
                $decimalx2 = decima5($this->inverse[1]);
            } elseif (($this->inverse[1]) < 10) {
                $decimalx2 = decima4($this->inverse[1]);
            } elseif (($this->inverse[1]) < 100) {
                $decimalx2 = decima3($this->inverse[1]);
            } elseif (($this->inverse[1]) < 1000) {
                $decimalx2 = decima2($this->inverse[1]);
            } elseif (($this->inverse[1]) < 10000) {
                $decimalx2 = decima1($this->inverse[1]);
            } else {
                $decimalx2 = decima0($this->inverse[1]);
            }


            ?>

            <h2 style="color:#F96010;">  <?php echo number_format(($this->value), 0) . " ";
                echo $this->base->code . " = " . $decimalx . ' ' . $this->defaultCurrency->code ?></h2>
            ...
            <table width="100%" cellspacing="5" cellpadding="5" align="center" class="praweokno">
                ...
                <tr>
                    <td class="currencytd">
                        ...
                        <p class="currencyone"> 1 <?php echo JText::_('MAIN_' . strtoupper($this->base->name)) ?>
                            (<?php echo $this->base->symbol ?>)
                            = <?php echo $decimalx1 ?> <?php echo $this->defaultCurrency->currency_name ?>
                            (<?php echo $this->defaultCurrency->symbol ?>)</p>
                    </td>
                    ...
                    <td class="currencytd">
                        ...
                        <p class="currencyone">
                            1 <?php echo JText::_('MAIN_' . strtoupper($this->defaultCurrency->name)) ?>
                            (<?php echo $this->defaultCurrency->symbol ?> ) = <?php echo $decimalx2 ?>
                ...
                <tr>
                    <td colspan="3">
                        ...
                        echo sprintf(JText::_('CURRENCIES_2'), "<strong>", "</strong>",
                            number_format(($this->value), 0, $com1, $com2),
                            JText::_('MAIN_' . strtoupper($this->base->name)), $this->base->code,
                            JText::_('MAIN_' . strtoupper($this->defaultCurrency->name)), $this->defaultCurrency->code,
                            date('Y-m-d H:i:s', strtotime($lastUpdateDate)), $link1, $link2, $this->base->name,
                            JText::_('MAINMC_' . strtoupper($this->base->name))); ?>
        ...
        <div class="row">
            ...
            <div class="col-sm-12">
                ...
                <h2><?php echo JText::_('MAJOR_WORLD_CURRENCIES') ?> </h2>
                    ...
                    $count = 0;
                    $time = 0;
                    $continentCur = ['popular' => [], 'america' => [], 'europe' => [], 'mea' => [], 'asia' => [], 'pacific' => [], 'africa' => []];
                    $cur_count = 0;
                    foreach ($this->historyRates as $hrate) {
                        ...
                        if (in_array($this->base->code, $os1)) {
                            ...
                            $decimal = decima4($hrate->rate / $currentRate1);
                        }
                        ...
                        switch ($hrate->continent) {
                            case 'Asia':
                                array_push($continentCur['asia'], [$decimal, $hrate]);
                                break;
                            case 'America':
                                array_push($continentCur['america'], [$decimal, $hrate]);
                                break;
                            case 'Europe':
                                array_push($continentCur['europe'], [$decimal, $hrate]);
                                break;
                            case 'Africa':
                                array_push($continentCur['africa'], [$decimal, $hrate]);
                                break;
                            case 'Pacific':
                                array_push($continentCur['pacific'], [$decimal, $hrate]);
                                break;
                            default:
                                array_push($continentCur['mea'], [$decimal, $hrate]);
                                break;
                        }
                        if ($hrate->main) {
                            array_push($continentCur['popular'], [$decimal, $hrate]);
                        }
                    }
                        ?>
                    ...
                    <ul class="nav nav-tabs">
                        <li class="active">
                            <a data-toggle="tab" href="#popular"><?php echo JText::_('MAIN_CURRENCIES'); ?></a>
                        </li>
                        <?php for ($c = 0; $c < count($continents); $c++) {
                            if ($continents[$c] != '') { ?>
                                <li>
                                    <a data-toggle="tab"
                                       href="#<?php echo strtolower($continents[$c]); ?>">
                                        <?php echo JText::_('MAIN_LA' . strtoupper(ucfirst($continents[$c]))); ?>
                                    </a>
                                </li>
                    ...
                    <div class="tab-content">
                        <?php
                        $start = microtime(true);
                        foreach ($continentCur as $tab => $value) {
                            ?>
                            <div role="tabpanel" class="tab-pane<?php if ($tab == 'popular') {echo ' active';}?>" id="<?php echo $tab ?>">
                                <div class="table-responsive">
                                    <table class="table table-striped table-bordered curr-table">
                                        <tr class="show">
                                            <th colspan="5">
                                                <?php
                                                echo JText::sprintf('CURRENCIES_1',
                                                    JText::_('MAIN_' . strtoupper($this->base->name)) . " (" . $this->base->code . ")",
                                                    date('Y-m-d H:i:s',
                                                        strtotime($lastUpdateDate)))//echo JText::sprintf('COM_CURRENCIES_EXCHANGE_RATES', JText::_('MAIN_'.strtoupper($this->base->name)), JText::_('MAIN_'.strtoupper($this->currency2->name)))
                                                ; ?>
                                                <br>
                                                <span class="tabelkacurr">
                                                    <?php echo sprintf(JText::_('CURRENCIES_5'), JText::_('MAIN_' . strtoupper($this->base->name)), "<strong>", "</strong>",
                                                        $this->base->name); ?>
                                                </span>
                                            </th>
                                        </tr>
                                            ...
                                            foreach ($value as $cur) { ?>
                                                <tr>
                                                    <td class="country firstColumn">
                                                        <img src="components/com_currencies/assets/images/blank.gif"
                                                        class="flag flag-<?php echo strtolower(substr($cur[1]->flag, 0,
                                                            2)); ?>"
                                                        alt="<?php echo $cur[1]->currency_name ?>"/>
                                                        <a href="<?php echo sprintf('/currencies/pairs/%s-%s-page.html', str_replace(' ', '-', strtolower($cur[1]->currency_name)), strtolower($cur[1]->currency2)); ?>">
                                                            <?php echo JText::_(JText::_('MAIN_' . strtoupper($cur[1]->currency_name))) . ' (' . $cur[1]->currency2 . ')' ?>
                                                        </a>
                                                    </td>
                                                    <td class="currencyex">
                                                        <a href="<?php echo sprintf('/currencies/pairs/1-%s-%s-to-%s-%s.html', str_replace(' ', '-', strtolower($this->base->name)), strtolower($this->base->code), str_replace(' ', '-', strtolower($cur[1]->currency_name)), strtolower($cur[1]->currency2)); ?>"><?php echo "1 ";
                                                        echo $this->base->code . " = " . $cur[0] . " " . $cur[1]->currency2 ?></a>
                                                    </td>
                                                    <td class="hidden-sm hidden-xs">
                                                        <a href="<?php echo sprintf('/currencies/pairs/%s-%s-calculator.html', str_replace(' ', '-', strtolower($cur[1]->currency_name)), strtolower($cur[1]->currency2)); ?>"><?php echo $cur[1]->currency2 . " " . JText::_('COM_CURRENCIES_SMCALCULATOR'); ?></a>
                                                    </td>
                                                    <td class="pairingex">
                                                        <a href="<?php echo sprintf('/currencies/pairs/10-%s-%s-to-%s-%s.html', str_replace(' ', '-', strtolower($this->base->name)), strtolower($this->base->code), str_replace(' ', '-', strtolower($cur[1]->currency_name)), strtolower($cur[1]->currency2)); ?>"><?php echo JText::_('COM_CURRENCIES_CONVERT_T') . " " . $this->base->code . " " . JText::_('COM_CURRENCIES_ON_THIS_TO') . " " . $cur[1]->currency2 ?></a>
                                                    </td>
                                                </tr>
                                            <?php }
                                            ?>
        ...
        <div class="countries">
            ...
            echo sprintf(JText::_('CURRENCIES_3'), JText::_('MAIN_' . strtoupper($this->base->name)), $this->base->code,
                $con); ?>
        ...
        <div class="countries">
            ...
            <?php echo sprintf(JText::_('CURRENCIES_4'), "<strong>", "</strong>",
                JText::_('MAIN_' . strtoupper($this->base->name)), $this->base->code); ?>


            <textarea rows="6" cols="20"
                      class="textareafoot">&lt;a href="http://<?php echo $_SERVER['SERVER_NAME'] . JRoute::_('index.php?option=com_currencies&view=pairs&base=' . $this->base->code . '&layout=landing&Itemid=' . $this->params->get('pairs_itemid')) ?>
                "&gt;<?php echo JText::_('MAIN_' . strtoupper($this->base->name)) . " (" . $this->base->code . ")" . " " ?> <?php echo JText::_('COM_CURRENCIES_EXCHANGE_RATESFOO') ?>
                &lt;/a&gt;</textarea>
        </div>
