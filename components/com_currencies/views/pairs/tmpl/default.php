...
$link3 = /*'<a href="' . JRoute::_('index.php?option=com_currencies&view=pairs&base=' . $this->base->code . '&to=' . $this->currency2->code . '&value=' . $this->value . '&Itemid=' . $this->params->get('pairs_itemid')) . '">' .*/ 
'<strong>'. $all->format($this->value) . $displaylakh. " " . $this->base->code . " " . JText::_('MAIN_TO') . " " . $this->currency2->code /*. '</a>'*/ . '</strong>';



if ($datelang=="hi_IN.UTF-8" || $datelang=="en_CA.UTF-8"|| $datelang=="en_NZ.UTF-8") { 

$link4 = '<a href="' . JRoute::_('index.php?option=com_currencies&view=pairs&base=' . $this->currency2->code . '&to=' . $this->base->code . '&value=' . number_format($this->value,countDecimals($this->value),".","") . '&Itemid=' . $this->params->get('pairs_itemid')) . '">' .
JText::_('CURRENCIES_16A') . " " . $all->format($this->value) . " " . $this->currency2->code . " " . JText::_('MAIN_TO') . " " . $this->base->code . '</a>'; }

else {$link4 = '<a href="' . JRoute::_('index.php?option=com_currencies&view=pairs&base=' . $this->currency2->code . '&to=' . $this->base->code . '&value=' . number_format($this->value,countDecimals($this->value),".","") . '&Itemid=' . $this->params->get('pairs_itemid')) . '">' .
JText::_('CURRENCIES_16A') . " " . $all->format($this->value) . " " . $this->currency2->code . " " . JText::_('MAIN_TO') . " " . $this->base->code . '</a>';}
...
{module Reklama_LINKI}
                ...
                <div class="row">
                    <div class="col-sm-12">
      <?php if (is_array($this->rates)&& count($this->rates) > 0) { ?>
                            <table class="table table-striped table-bordered">
                            <tr>
                                <th colspan="3">
...
//$linkXX=str_replace(",",".",$linkXX);

$linkXX = '<a href="' . JRoute::_('index.php?option=com_currencies&view=pairs&base=' . $this->base->code . '&to=' . $this->currency2->code . '&value=' . number_format($this->value,countDecimals($this->value),".","") . '&Itemid=' . $this->params->get('pairs_itemid')) . '">' . $all->format($this->value). " ".  $this->base->code . '</a>';

echo $linkXX=str_replace(",",".",$linkXX);
...
if (($currRate[1] * $this->value) >= 1000) {$decimalxy=0;}
if (($currRate[1] * $this->value) >= 100 && ($currRate[1] * $this->value) < 1000) {$decimalxy=1;}
if (($currRate[1] * $this->value) >= 10 && ($currRate[1] * $this->value) < 100) {$decimalxy=2;}
if (($currRate[1] * $this->value) >= 1 && ($currRate[1] * $this->value) < 10) {$decimalxy=4;}
if (($currRate[1] * $this->value) < 1) {$decimalxy=6;}	
  
	  
	  
$linkvalue = '<a href="' . JRoute::_('index.php?option=com_currencies&view=pairs&base=' . $currencyvalue->code . '&to=' . $this->base->code . '&value=' . number_format($currRate[1] * $this->value,$decimalxy,".","") . '&Itemid=' . $this->params->get('pairs_itemid')) . '">' . $decimalf4. " ".$funt2 . " (" .$currencyvalue->code .") " . '</a>';

echo str_replace(",",".",$linkvalue);
