...
class CurrenciesViewPairs extends JViewLegacy
{
	...
	public function display($tpl = null)
	{
		...
		$base                    = strtolower($app->input->get('base', '', 'string'));
		...
		$this->defaultCurrency   = strtolower($currenciesModel->getByCode($this->params->get('primary_currency'))->name);
		$this->allCurrencies     = $currenciesModel->getCurrencies();
		$glang=$this->params->get('component_lang');
		
	//echo  "<br>".
	$walutaz = $this->params->get('primary_currency');	// echo $layout; echo $base;

if ($walutaz == 'BDT') {		
		
		if ($layout == 'landing' && ($base == 'bdt' || $base == 'BDT'))
		{ $this->defaultCurrency = $currenciesModel->getByCode('USD');}
		else
		{ $this->defaultCurrency = $currenciesModel->getByCode('BDT');}
				}
elseif ($walutaz == 'TRY') {		
		
		if ($layout == 'landing' && ($base == 'try' || $base == 'TRY'))
		{ $this->defaultCurrency = $currenciesModel->getByCode('USD');}
		else
...
elseif ($walutaz == 'RON') {		
		...
		if ($layout == 'landing' && ($base == 'ron' || $base == 'RON'))
		{ $this->defaultCurrency = $currenciesModel->getByCode('EUR');}
		else
		{ $this->defaultCurrency = $currenciesModel->getByCode('RON');}
				}		


elseif ($walutaz == 'EUR') {		
		
		if ($layout == 'landing' && ($base == 'eur' || $base == 'EUR'))
		{ $this->defaultCurrency = $currenciesModel->getByCode('USD');}
		else
		{ $this->defaultCurrency = $currenciesModel->getByCode('EUR');}
				}				
...
elseif ($walutaz == 'USD') {		
		...
		if ($layout == 'landing' && ($base == 'usd' || $base == 'USD'))
		{ $this->defaultCurrency = $currenciesModel->getByCode('EUR');}
		else
		{ $this->defaultCurrency = $currenciesModel->getByCode('EUR');}
				}					
