...
class CurrenciesModelCurrency extends JModelItem
{
   ...
   function getById($id){
    ...
    public function getLastUpdateDate()
    {
        ...
        $query = 'select date from #__currencies_rates_current limit 1';
        $db->setQuery($query);
    ...
    public function getAllCurrencies()
    {
	    $db = JFactory::getDbo();
	    $query = $db->getQuery(true);
	    $query->select($db->quoteName(array('name', 'code')));
	    $query->from($db->quoteName('#__currencies_all_currencies'));

	    $db->setQuery($query);

	    $session = JFactory::getSession();
	    $session->set('allCurrencies', $db->loadObjectList());
	    return true;
    }
    ...
    {
	    $session = JFactory::getSession();
	    $allCurrencies = $session->get('allCurrencies');
