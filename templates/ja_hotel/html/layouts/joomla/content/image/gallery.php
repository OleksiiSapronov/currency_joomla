...
if(is_array($gallery) && isset($gallery['src'])) {
	...
	if(count($gallery['src']) > 1) {
      ...
      $doc->addStyleSheet(T3_TEMPLATE_URL . '/js/gallery/blueimp-gallery.min.css');
      $doc->addStyleSheet(T3_TEMPLATE_URL . '/js/gallery/bootstrap-image-gallery.css');
      $doc->addScript(T3_TEMPLATE_URL . '/js/gallery/jquery.blueimp-gallery.min.js');
      $doc->addScript(T3_TEMPLATE_URL . '/js/gallery/bootstrap-image-gallery.min.js');
	...
	<?php if (isset($thumbnail) && !empty($thumbnail)) : ?>
		...
		<?php if(is_array($gallery) && count($gallery['src']) > 1):
			...
			foreach($gallery['src'] as $index => $src) {
				...
				$img->href = JUri::root(true).'/'.$src;
				$img->title = $gallery['caption'][$index];
				if(!empty($gallery['link'][$index])) {
					//@todo add link for gallery item
				}
				$images[] = $img;
			}
			?>
