...
function modChrome_T3isotope($module, &$params, &$attribs)
{ 
	...
	if (!empty ($module->content)) {
		...
		if ($module->showtitle != 0) {
			$html .= "<div class=\"module-title item isotope-item {$moduleClassSfx}\"><div class=\"item-inner\"><span>{$module->title}</span>{$moduleIntro}</div></div>";
		}
...
{ 
  ...
  $moduleIntro   = $params->get('module-intro');
	...
	if (!empty ($module->content)) {
		...
		if ($module->showtitle != 0) {
			$html .= "<{$headerTag} class=\"module-title {$headerClass}\"><span>{$module->title}</span><span class=\"module-intro\">{$moduleIntro}</span></{$headerTag}>";
		}
